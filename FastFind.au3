...
global $FFDefautDebugMode = 0x00 ; Si below to the meaning of this value. To remove all debug features (file traces, graphical feedback..., use 0 here)
; Factory default = 0xE7 (-B.J. Black)
...
; B.J. Black: A global variable to hold the screen xy-position of the shots, because I take shots like that, but want to code it out.
GLOBAL $FFSnapOrigin[$FFNbSnapMax][2]
...
Func InitFFDll()
	...
	; Returns 100 if the attempt to open the dll fails.
	; Returns 101 if the version number of the dll is different from the version string of this script.
	
	for $i = 0 To $FFNbSnapMax-1
		$FFLastSnapStatus[$i] = 0
	Next
	If @AutoItX64 Then
		local $DllName = "FastFind64.dll"
	Else
		local $DllName = "FastFind.dll"
	Endif
	...
	if $FFDllHandle = -1 then
		...
		if NOT @compiled then
			$DllName = RegRead ("HKEY_LOCAL_MACHINE\SOFTWARE\AutoIt v3\AutoIt", "InstallDir") & "\Include\" & $DllName
;~ 			ConsoleWrite ($DllName & @CRLF)
			$FFDllHandle = DllOpen($DllName)
		endif
		
		If $FFDllHandle=-1 Then
;~ 			MsgBox(0,"Error","Failed to load "&$DllName&", application probably won't properly work. "&@CRLF&"Check if the file "&$DllName&"is installed near this script")
			Return 100
		EndIf
	endif
...
; ===============================================================================================================================
; B.J. Black: An error returning 0 may be a failure to initialize the FF dll.
; ===============================================================================================================================
Func FFSnapShot(const $Left=0, const $Top=0, const $Right=0, const $Bottom=0, const $NoSnapShot=$FFDefaultSnapShot, const $WindowHandle=-1)
	...
	If (Not IsArray($Res) AND $Res=0) OR $Res[0]=0 Then
		local $msg = "SnapShot ("&$Left&","&$Top&","&$Right&","&$Bottom&","&$NoSnapShot&","&Hex($WindowHandle,8)&") failed." & @CRLF
		if (IsArray($Res)) Then
			$msg &= "IsArray($Res):"&IsArray($Res)&" - Ubound($Res):"&UBound($Res)&" - $Res[0]:"&$Res[0]
		else
			$msg &= "IsArray($Res):"&IsArray($Res)&" - $Res:"&$Res
		EndIf
...
; ===============================================================================================================================
; B.J. Black: Added the $FFSnapOrigin shift. May break scripts, but since $FFSnapOrigin is only set manually by the user, it shouldn't be a problem for others.
; ===============================================================================================================================
Func FFGetPixel($x, $y, $NoSnapShot=$FFLastSnap)
	local $Result = DllCall($FFDllHandle, "int", "FFGetPixel", "int", $x + $FFSnapOrigin[$NoSnapShot][0], "int", $y + $FFSnapOrigin[$NoSnapShot][1], "int", $NoSnapShot)
	If ( (Not IsArray($Result)) or ($Result[0]=-1) ) Then
		SetError(2)
		Return -1
	EndIf
